...
@connect(({ machinery,eInto,project, loading }) => ({
    machinery,eInto,project,
    loading: loading,
}))
@Form.create()
export default class eInto extends PureComponent {
    state = {
        list: [],
        expandForm: false,
        isShowForm: false,
...
class NewForm extends React.Component {
    ...
    componentDidMount() {
        console.log('fetchMachinery  NewForm extends React.Component')
        this.fetchProject();
        this.fetchMachinery();
    }
    fetchProject=()=>{
        ...
        return (
            <Form layout="horizontal" disabled='true' >
                ...
                <Form.Item label="进场日期" {...formItemLayout}>
                    {
                        formInfo && type == 'intoDate' ? formInfo.intoDate :
                            getFieldDecorator('intoDate', {
                                initialValue: formInfo.intoDate == null ? moment(moment().locale('zh-cn').format('YYYY-MM-DD'), 'YYYY-MM-DD') : moment(formInfo.intoDate, 'YYYY-MM-DD'),
                                rules: [{
                                    required: true, message: '请选进场日期',
                                }],
                            })(
